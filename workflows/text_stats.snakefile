...
if not os.path.exists(PYTHON_ENV):
    ...
    PYTHON_ENV = f'source {PYTHON_ENV}; '
...
rule all:
    input: 'data/gcdc.lda_coherence.csv',
        'data/pmc_articles.all_readability_scores.csv',
        'data/pmc_articles.lda_coherence.csv'
...
rule merge_lda_stats:
    ...
    output: 'data/pmc_articles.lda_coherence.csv'
    log: 'data/pmc_articles.snakemake.merge_lda_coherence.log'
    shell: PYTHON_ENV + ' python scripts/merge_csv_files.py'
        + ' --input_files_pattern "data/pmc_articles/*.lda_coherence.csv"'
        + ' --output_file {output} &> {log}'
